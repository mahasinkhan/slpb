...
const DashboardOverview: React.FC<{ theme: Theme }> = ({ theme }) => {
  ...
  const trafficData = [
    ...
    { time: '12:00', users: 567 },
    { time: '16:00', users: 489 },
    { time: '20:00', users: 312 },
    { time: '23:59', users: 198 }
  ];
    ...
    <div className={`p-4 sm:p-6 lg:p-8 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'} min-h-screen`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {stats.map((stat, index) => (
          <div
            key={index}
            className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} border rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all`}
          >
            ...
            <h3 className={`text-3xl font-black mb-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
              {stat.value}
            </h3>
        ...
        <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} border rounded-2xl p-6 shadow-lg`}>
          ...
          <div className="space-y-3">
            {systemAlerts.map((alert, idx) => (
              <div
                key={idx}
                className={`p-3 border-l-4 rounded ${isDarkMode ? 'bg-gray-700' : getSeverityColor(alert.severity)}`}
              >
      ...
      <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} border rounded-2xl p-6 shadow-lg`}>
        ...
        <div className="space-y-3">
          {recentActivity.map((activity, idx) => (
            <div
              key={idx}
              className={`flex items-start justify-between p-4 rounded-xl ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}
            >
...
const SidebarItem: React.FC<SidebarItemProps> = ({ item, isActive, isCurrentPage, onClick, theme, currentPage }) => {
  ...
  const childActive = isDarkMode
    ? 'bg-blue-800/60 text-blue-200 font-semibold border-l-4 border-blue-500'
    : 'bg-blue-100 text-blue-800 font-semibold';
  const childInactive = isDarkMode
  ...
  return (
    <div className="w-full group">
      <button
        ...
        className={`${buttonBase} ${isCurrentPage && !item.children ? buttonActive : buttonInactive}`}
      >
        ...
        {item.children && (
          <ChevronRight className={`w-4 h-4 transition-transform duration-300 ${isOpen ? 'rotate-90' : 'rotate-0'} ${isCurrentPage ? 'text-white' : ''}`} />
        )}
      </button>
        ...
        <div className={`transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-96 opacity-100 pt-1' : 'max-h-0 opacity-0'}`}>
          <div className={`pl-8 border-l ml-5 pt-1 ${isDarkMode ? 'border-gray-700' : 'border-blue-100'}`}>
            {item.children.map((child) => (
              <button
                key={child.id}
                onClick={() => onClick(child.id as RouteKey)}
                className={`${childBase} ${child.id === currentPage ? childActive : childInactive}`}
              >
    ...
    <div className={`flex min-h-screen transition-colors duration-500 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-800'}`}>
      ...
      <div className="lg:hidden fixed top-4 left-4 z-50">
        <button 
          ...
          className="p-3 bg-blue-600 text-white rounded-xl shadow-lg transition-all hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300"
        >
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 flex-shrink-0 shadow-2xl transition-transform duration-300 ease-in-out
          ${isDarkMode ? 'bg-gray-800 border-r border-gray-700' : 'bg-white shadow-xl border-r border-gray-100'}
          ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex flex-col h-full p-4 overflow-y-auto">
          <div className="p-2 mb-6">
            <h2 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-400">
              SL Brothers
            </h2>
          ...
          <nav className="space-y-1 flex-grow">
            {NAV_ITEMS.map((item) => (
              <SidebarItem
                key={item.id}
                item={item}
                isCurrentPage={item.id === currentPage}
                isActive={isGroupActive(item.id)}
          ...
          <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
            <button className="flex items-center w-full p-3 text-sm font-medium transition-all duration-200 rounded-xl text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 dark:hover:text-red-400">
      ...
      {/* Mobile Overlay */}
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-black opacity-50 z-30 lg:hidden" onClick={() => setIsSidebarOpen(false)}></div>
      )}

      {/* Main Content */}
      <main className="flex-grow overflow-y-auto w-full">
        {/* Header */}
        <header className={`sticky top-0 z-10 mb-0 p-4 shadow-lg transition-colors duration-500
            flex justify-between items-center 
            ${isDarkMode ? 'bg-gray-800/90 backdrop-blur-md border-b border-gray-700' : 'bg-white/95 backdrop-blur-md border-b border-gray-100'}`}>
          <h1 className="text-2xl lg:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-500">
            {getCurrentPageTitle}
          </h1>
          <div className="flex items-center space-x-3 lg:space-x-4">
            <button
                onClick={toggleTheme}
                className={`p-2 rounded-full transition-all duration-300 focus:outline-none focus:ring-4 
                  ${isDarkMode ? 'text-yellow-400 hover:bg-gray-700 ring-gray-700' : 'text-blue-600 hover:bg-blue-50 ring-blue-100'}`}
                aria-label="Toggle Theme"
            >
                {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>

            <span className={`text-sm font-medium hidden md:inline transition-colors duration-300 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                Welcome, SA
            </span>
            <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center text-white font-bold shadow-md ring-2 ring-blue-300 dark:ring-blue-500/50">
              SA
            </div>
          </div>
        </header>
        ...
        <section className="min-h-[calc(100vh-80px)]">
            <ContentRenderer key={contentKey} currentPage={currentPage} theme={theme} />
        </section>
      </main>
    </div>
  );
